 ...
 */
class SuitTwigExtension extends \Twig_Extension {
  /**
   * {@inheritdoc}
   * This function must return the name of the extension. It must be unique.
   */
  public function getName() {
    return 'emulsify_twig_suit';
  }
  ...
  public function getFunctions() {
    return array(
      new \Twig_SimpleFunction('suit', array($this, 'suit'), array('needs_context' => true, 'is_safe' => array('html'))),
    );
  }
   ...
   */
  public function suit($context, $base_class, $modifiers = array(), $blockname = '', $extra = array()) {
    $classes = [];
    ...
    if (is_object($base_class) || is_array($base_class)) {
      ...
      if ($blockname) {
        // Set blockname class.
        $classes[] = $blockname . '-' . $base_class;
        // Set blockname--modifier classes for each modifier.
        if (!empty($modifiers)) {
          foreach ($modifiers as $modifier) {
            $classes[] = $blockname . '-' . $base_class . '--' . $modifier;
          };
        }
      }
      ...
      // If extra non-SUIT classes are added.
      if (!empty($extra)) {
